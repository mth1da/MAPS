...
class JWTService
{
     ...
     * @return string
     */
    public function generate(array $header, array $payload, string $secret, int $validity = 10800): string //10800s = 3h
    {
        if ($validity > 0) {
    ...
    public function check(string $token, string $secret)
    {
        ...
        // on régénère un token pour vérifier la signature
        $verifToken = $this->generate($header, $payload, $secret, 0);

        return $token === $verifToken; //true = token pas corrompu
    }
